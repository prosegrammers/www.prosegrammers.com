...
echo: true
description: "Understanding RAG concepts and building simple implementations"
date: "2025-12-01"
date-format: long
...
- {{< iconify fa6-solid book >}} **What are this week's highlights?**
    - A "from scratch" implementation of RAG concepts:
...
build intelligent document systems that provide contextual answers
- {{< iconify fa6-solid book >}} Simple implementations using basic Python
can demonstrate core RAG concepts without complex libraries
- {{< iconify fa6-solid code >}} Understanding RAG fundamentals helps
prosegrammers design better document-based applications
...
- {{< iconify fa6-solid bullhorn >}} This week's content aids in the
attainment of learning objectives **CS-104-3** and **CS-104-4**!
...
- **Key concept**: RAG starts with loading documents into memory
...
doc = "RAG systems retrieve documents. They combine results with
generation. This creates better answers."
...
doc = "Document engineering combines programming skills with writing
skills for creating better documentation systems"
...
# Creating vector-like representations
...
- {{< iconify fa6-solid box >}} **Popular embedding tools**:
...
- {{< iconify fa6-solid lightbulb >}} **For this course**: We use simple
word-based representations to understand concepts without external
dependencies
...
- Represent text as word frequency dictionary
- Each word becomes a feature dimension
- **Simple but demonstrates vectorization concept**
...
def retrieve_chunks(query: str, chunks: List[str], top_k: int = 2) ->
...
chunks = ["Document engineering combines programming and writing",
"Prosegrammers use Python for document analysis", "RAG systems retrieve
relevant documents"]
...
- Return top matches
- **Demonstrates retrieval mechanism**
...
explain_relevance("document tools", "Document engineering creates tools
for writers")
```
...
chunks = ["Document engineering combines programming with writing",
"Prosegrammers build document analysis tools"]
...
#| autorun: true
#| max-lines: 8
def format_context_with_sources(query: str, chunks: List[Tuple[str,
float]]) -> str:
...
retrieved = retrieve_chunks("prosegrammer skills", ["Prosegrammers
combine coding and writing", "Document tools require technical skills"],
top_k=2)
...
#| autorun: true
#| max-lines: 9
def generate_response(query: str, retrieved: List[Tuple[str, float]]) ->
str:
...
query = "What is document engineering?"
chunks = ["Document engineering combines programming with technical
writing", "Tools help automate document creation"]
...
#| autorun: true
#| max-lines: 10
def generate_detailed_response(query: str, retrieved: List[Tuple[str,
float]]) -> str:
...
chunks = ["Prosegrammers combine programming and writing skills",
"Document tools automate content creation", "Analysis techniques improve
document quality"]
retrieved = retrieve_chunks(query, chunks, top_k=2)
...
#| autorun: true
#| max-lines: 10
def simple_rag_system(documents: List[str], query: str, top_k: int = 2)
-> str:
...
docs = ["Document engineering uses Python to analyze text. Tools help
automate writing tasks.", "Prosegrammers combine programming skills with
technical writing. They create better documentation systems."]
...
docs = ["RAG systems retrieve relevant documents", "Retrieved
information grounds generated responses", "Context improves answer
quality"]
...
# Building better document systems
...
- {{< iconify fa6-solid lightbulb >}} **How would you enhance a RAG
system for your document engineering projects?**
...
- {{< iconify fa6-solid lightbulb >}} **Apply RAG to document engineering**
  - Build question-answering systems
  - Create documentation assistants
  - Develop knowledge base applications
  - Simple implementations demonstrate core concepts

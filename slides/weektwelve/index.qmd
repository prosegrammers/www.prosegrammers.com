...
- {{< iconify fa6-solid lightbulb >}} **Can you write an improved regex for email
addresses?**
...
## Character classes in regex {transition="convex"}
...
- {{< iconify fa6-solid lightbulb >}} **Modification**: change the text to
include your own suitable emails
- {{< iconify fa6-solid text-slash >}} **Exploration**: identify some email
addresses that are not detected
- {{< iconify fa6-solid rocket >}} **Extension**: try to make pattern detection for
emails more robust
...
- {{< iconify fa6-solid filter >}} **Validate input**: confirm format
compliance
...
in documents
- {{< iconify fa6-solid rocket >}} **Confirm correctness**: test to be
...
# remove markdown bold syntax with non-greedy matching
no_bold = re.sub(r'\*\*(.+?)\*\*', r'\1', markdown_text)

# remove markdown italic syntax using non-greedy matching
no_italic = re.sub(r'\*(.+?)\*', r'\1', no_bold)

# remove markdown links, keep text, use first capture group
...
examples
- {{< iconify fa6-solid bug >}} **Edge cases**: empty strings or special
characters
...
engineering tools reliable and maintainable! Please test all methods that use
regular expressions!
...
{{< iconify fa6-solid lightbulb >}} Testing regex is essential because patterns
can be complex and hard to understand --- subtle bugs can hide in tricky edge
cases!

:::

# Benefits and limitations of regular expressions {transition="convex"}

::: {.fragment style="margin-top: -0.5em; font-size: 0.7em;"}
...
- {{< iconify fa6-solid rocket >}} **Again, write tests for all functions using
regex**!
...
::: {.fragment .fade style="margin-top: -0.45em; font-size: 0.725em;"}
...
- {{< iconify fa6-solid gears >}} **Next steps with regular expression techniques**:
  ...
  - {{< iconify fa6-solid gears >}} **Combine multiple patterns for powerful
  document tools**
  - {{< iconify fa6-solid lightbulb >}} **How would regex make your document
